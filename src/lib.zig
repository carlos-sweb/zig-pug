...
// Wrapper for pretty-print that handles include_comments parameter
// Note: The current formatter doesn't support filtering comments,
// so this parameter is ignored for now. Future enhancement.
fn prettyPrintHtmlWithComments(allocator: std.mem.Allocator, html: []const u8, include_comments: bool) ![]const u8 {
    _ = include_comments; // TODO: Implement comment filtering in formatter
    return formatter.prettyPrintHtml(allocator, html);
}
...
export fn zigpug_pretty_print(html: [*:0]const u8, include_comments: c_int) ?[*:0]u8 {
    ...
    const formatted = prettyPrintHtmlWithComments(allocator, html_str, include_comments != 0) catch return null;
...
export fn zigpug_minify(html: [*:0]const u8) ?[*:0]u8 {
    ...
    const minified = formatter.minifyHtml(allocator, html_str) catch return null;
