...
pub const Compiler = struct {
    ...
    fn compileConditional(self: *Self, node: *ast.AstNode) !void {
        ...
        // If the variable is not defined, mujs throws a ReferenceError.
        // We wrap the expression with typeof to avoid throwing — same behavior
        // as pug where undefined variables are treated as falsy.
        //
        // Strategy: try direct eval first. If it fails (ReferenceError or any
        // error), fall back to typeof check which never throws.
        //
        // typeof hero !== "undefined" && hero
        // → returns false/undefined safely when hero is not defined
        const safe_condition = try std.fmt.allocPrint(
            self.allocator,
            "(function(){{ try {{ var _v=({s}); return _v; }} catch(e) {{ return false; }} }})()",
            .{transformed_condition},
        );
        defer self.allocator.free(safe_condition);

        const result = self.runtime.eval(safe_condition) catch {
            // If even the safe wrapper fails, treat as falsy — never error out
            const should_execute = cond.is_unless; // unless false = true, if false = false
            if (should_execute) {
        ...
        // Check if result is truthy — same rules as JavaScript
    ...
    fn compileLoop(self: *Self, node: *ast.AstNode) !void {
        ...
        // Get the iterable value from runtime — wrap in safe IIFE to handle
        // undefined variables gracefully (same behavior as pug).
        const safe_iterable = try std.fmt.allocPrint(
            self.allocator,
            "(function(){{ try {{ var _v=({s}); return _v; }} catch(e) {{ return []; }} }})()",
            .{transformed_iterable},
        );
        defer self.allocator.free(safe_iterable);

        const iterable_result = self.runtime.eval(safe_iterable) catch {
            // If even the safe wrapper fails, treat as empty — run else branch
        ...
        const length_expr = try std.fmt.allocPrint(self.allocator, "({s}).length", .{safe_iterable});
    ...
    fn compileCase(self: *Self, node: *ast.AstNode) !void {
        ...
        // Evaluate the case expression — wrap in safe IIFE to handle
        // undefined variables gracefully (falls through to default branch).
        const safe_expr = try std.fmt.allocPrint(
            self.allocator,
            "(function(){{ try {{ var _v=({s}); return String(_v); }} catch(e) {{ return '__undefined__'; }} }})()",
            .{case_node.expression},
        );
        defer self.allocator.free(safe_expr);

        const case_value = self.runtime.eval(safe_expr) catch {
            // If even the safe wrapper fails, run default branch only
            if (case_node.default) |*default_branch| {
                for (default_branch.items) |child| {
                    try self.compileNode(child);
                }
            }
            return;
